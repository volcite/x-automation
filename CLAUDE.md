...
| テーマ差し込み | `bash post/scripts/pipeline_inject.sh '{"topic":"テーマ","details":"詳細","source_url":"URL","duration_days":3}'` |
| 朝パイプライン手動実行 | `bash post/scripts/pipeline_morning.sh` |
| リプライ返信手動実行 | `bash post/scripts/pipeline_reply.sh <replies.json>` |
| 分析パイプライン実行 | `bash post/scripts/pipeline_analysis.sh` |
