...
| 知見の使い方 | 条件 | 投稿への反映度 |
|---|---|---|
| **核として使用** | `priority` が `"high"` かつ `trends.json` のトレンドと掛け合わせられる場合 | 投稿のメインメッセージ・視点に据える。トレンド（事実）を、オーナーの思想（解釈）で語る企画にする |
| **味付けとして使用** | `priority` が `"medium"` の場合、または差し込みテーマが存在する場合 | 投稿の中で具体エピソード・引用・補強として1-2箇所で使用する |
| **使用しない** | 当日のトレンドが非常に強い場合、またはストックが空の場合 | トレンドのみで企画する（従来どおり） |
...
**差し込みテーマとの優先順位:**
1. `injection`（差し込みテーマ）→ 最優先
2. `knowledge (priority=high)` + トレンドマッチ → 高優先
3. トレンド単体 → 通常運用
4. `knowledge (priority=medium/low)` 単体 → ネタ切れ時のフォールバック
